...
class Denora_DB extends DB {
    ...
    public static function getConfig(): array {
        self::$config ??= MagircConfigStore::load('denora', PATH_ROOT . 'conf');
        return self::$config;
    }
...
class Denora implements Service {
    ...
    private function getPieStats($type, $mode = null, $target = null) {
        return $this->statisticsCache->remember(
            'pie_stats',
            ['type' => $type, 'mode' => $mode, 'target' => $target],
            fn () => $this->getPieStatsUncached($type, $mode, $target),
            false,
            $this->cacheScope($mode, $target)
        );
    }

    private function getPieStatsUncached($type, $mode = null, $target = null) {
        $query = "SELECT ";
        if ($type == 'clients') {
